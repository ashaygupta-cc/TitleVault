...
const RegistryMerkleAnchorPanel = () => {
  ...
  const fetchMerkleRoot = async () => {
    try {
      const data = await adminApi.getRegistryMerkleRoot();
      setMerkleData(data);
    } catch (error: any) {
  ...
  const handleAnchorMerkleRoot = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await adminApi.anchorRegistryMerkleRoot();
      ...
      toast({
        title: 'Success',
        description: 'Merkle root anchored on blockchain! ✓',
      });
    } catch (error: any) {
  ...
  return (
    <div className="p-6 space-y-6 overflow-auto registry-scrollbar h-full">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-semibold text-foreground">Registry Merkle Root Anchor</h2>
        <Button
          variant="outline"
          size="sm"
      ...
      <Card className="border-border bg-card">
        <CardHeader className="pb-4">
          ...
          <CardDescription>Latest merkle root of all registry records</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {isFetching ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-muted/30 rounded-lg">
                  <p className="text-xs text-muted-foreground mb-1">Records in Tree</p>
            ...
            <div className="text-center py-8">
              <p className="text-sm text-muted-foreground">No merkle root available</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <Card className="border-blue-500/30 bg-blue-500/5">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              This will submit the merkle root hash to the blockchain. Once anchored, all records in this tree can be verified against the on-chain root.
            </p>
      ...
      <Card className="bg-muted/30">
        <CardContent className="pt-4">
          <div className="space-y-2 text-sm">
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-3">How Merkle Root Anchoring Works</p>
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li className="flex gap-2">
                <span className="font-semibold">1.</span>
                <span>Registry records are organized into a Merkle tree structure</span>
              </li>
              <li className="flex gap-2">
                <span className="font-semibold">2.</span>
                <span>The root hash represents all records in the tree</span>
              </li>
              <li className="flex gap-2">
                <span className="font-semibold">3.</span>
                <span>Anchoring the root on blockchain makes all records verifiable</span>
              </li>
              <li className="flex gap-2">
                <span className="font-semibold">4.</span>
                <span>Each record can prove it's part of the anchored tree</span>
              </li>
            </ul>
          </div>
        </CardContent>
...
export default RegistryMerkleAnchorPanel;
